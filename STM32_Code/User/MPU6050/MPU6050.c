...
uint8_t MPU6050_Write_Len(uint8_t addr,uint8_t reg,uint8_t len,uint8_t *buf)
{
	uint8_t i; 
    IO_IIC_start(); 
	IO_IIC_write_byte((addr<<1)|0);//发送器件地址+写命令	
	if(MPU6050_IIC_Wait_Ack())	//等待应答
	{
		IO_IIC_stop();		 
		return 1;		
	}
    IO_IIC_write_byte(reg);	//写寄存器地址
    MPU6050_IIC_Wait_Ack();		//等待应答
	for(i=0;i<len;i++)
	{
		IO_IIC_write_byte(buf[i]);	//发送数据
...
uint8_t MPU6050_Read_Len(uint8_t addr,uint8_t reg,uint8_t len,uint8_t *buf)
{
    ...
    MPU6050_IIC_Wait_Ack();		//等待应答
    IO_IIC_start();
	IO_IIC_write_byte((addr<<1)|1);//发送器件地址+读命令	
    MPU6050_IIC_Wait_Ack();		//等待应答 
	while(len)
	{
		if(len==1)*buf=IO_IIC_read_byte(0);//读数据,发送nACK 
		else *buf=IO_IIC_read_byte(1);		//读数据,发送ACK  
		len--;
		buf++; 
	}    
    IO_IIC_stop();	//产生一个停止条件 
	return 0;	
}
