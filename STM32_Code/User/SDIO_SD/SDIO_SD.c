/**********************************************************************************************           
  ...
  *          +-----------------------------+---------------+-------------+  
	*
	*         1、 //注：擦除后是0xff或0x00    startaddr必须是512的整数倍 
	*         2、//需要在主函数定义的变量：    SD_Error Status = SD_OK;//用于存储该底层函数返回的状态
	*																					 extern SD_CardInfo SDCardInfo;//当需要调用结构体SD_CardInfo 存放的SD卡的信息时定义
	*         3、//需要在中断函数中写入这段代码 
	...
	*							}
	*         4、若需要改变DMA中断的优先级   可以在  SD_Init() 函数中修改
	*
	*         （对于这个底层  并没有完全理解 目前只理解了初始化部分  对于使用ＤＭＡ传送数据那块　　　还没有很好的理解）
	*
	*  SD_Error SD_Init(void)//初始化SD卡，使卡处于就绪状态(准备传输数据) 
	*  SD_Error SD_Erase(uint32_t startaddr, uint32_t endaddr);// SD_Error: 成功后会返回   SD_OK          
  *
//---------------------------------------------------------------------//
//---------------------单块空间的读数据------------------------------------//	
 
	    SD_Error SD_ReadBlock(uint8_t *readbuff, uint32_t ReadAddr, uint16_t BlockSize)//startaddr必须是512的整数倍  
			SD_WaitReadOperation();  //等待DMA传输结束
			while(SD_GetStatus() != SD_TRANSFER_OK);// 查询接收是否结束    完成后会返回  SD_TRANSFER_OK
 
//---------------------------------------------------------------------//
//---------------------单块空间的写数据------------------------------------//
	    SD_WriteBlock(uint8_t *writebuff, uint32_t WriteAddr, uint16_t BlockSize);  //startaddr必须是512的整数倍  
			SD_WaitWriteOperation(); //等待DMA传输结束
			while(SD_GetStatus() != SD_TRANSFER_OK); // 查询传输是否结束    完成后会返回  SD_TRANSFER_OK	
//------------------------------------------------------------------------//
//---------------------多块空间的读数据------------------------------------// 			
...
SD_CardInfo SDCardInfo;	  //用于存储卡的信息，DSR的一部分  在初始化函数调用后SD卡的信息会自动存储在这个结构体中
...
static uint32_t CSD_Tab[4], CID_Tab[4], RCA = 0;//存储CSD，CID，寄存器和卡相对地址
...
void SD_DeInit(void)
{
  ...
  SDIO_DeInit();//SDIO模块函数/*!< DeInitializes the SDIO peripheral */
 ...
 * 输出  ：无
 */
void NVIC_Configuration(void)//抢占优先级为0   响应优先级为4
{
...
/*************************************************************************
*  函数名称：Display_SD_Information
*  功能说明：通过串口打印SD卡的信息   若初始化不成功则不打印
*  参数说明：无
*             
*  函数返回：无
*  修改时间：2014-12-25
 ...
 void Display_SD_Information(void)
 {
    ...
    UART_send_string(USART1," \r\n CardType is ： ");//字符串函数
	  ...
	  if(SDCardInfo.CardType == SDIO_STD_CAPACITY_SD_CARD_V1_1)
		{
        UART_send_string(USART1,"SDIO_STD_CAPACITY_SD_CARD_V1_1");//字符串函数
    }
		else if(SDCardInfo.CardType == SDIO_STD_CAPACITY_SD_CARD_V2_0)
		{ 
        UART_send_string(USART1,"SDIO_STD_CAPACITY_SD_CARD_V2_0");//字符串函数
    }
		else if(SDCardInfo.CardType == SDIO_HIGH_CAPACITY_SD_CARD)
		{ 
        UART_send_string(USART1,"SDIO_HIGH_CAPACITY_SD_CARD");//字符串函数
    }
		else if(SDCardInfo.CardType == SDIO_MULTIMEDIA_CARD)
		{ 
        UART_send_string(USART1,"SDIO_MULTIMEDIA_CARD");//字符串函数
    }
		else if(SDCardInfo.CardType == SDIO_SECURE_DIGITAL_IO_CARD)
		{ 
        UART_send_string(USART1,"SDIO_SECURE_DIGITAL_IO_CARD");//字符串函数
    }	
		else if(SDCardInfo.CardType == SDIO_HIGH_SPEED_MULTIMEDIA_CARD)
		{ 
        UART_send_string(USART1,"SDIO_HIGH_SPEED_MULTIMEDIA_CARD");//字符串函数
    }	
		else if(SDCardInfo.CardType == SDIO_SECURE_DIGITAL_IO_COMBO_CARD)
		{ 
        UART_send_string(USART1,"SDIO_SECURE_DIGITAL_IO_COMBO_CARD");//字符串函数
    }
		else if(SDCardInfo.CardType == SDIO_HIGH_CAPACITY_MMC_CARD)
		{ 
        UART_send_string(USART1,"SDIO_HIGH_CAPACITY_MMC_CARD");//字符串函数
    }
		else ;
		
		
	  UART_send_string(USART1," \r\n CardBlockSize is :  ");//字符串函数
	  UART_send_data(USART1,SDCardInfo.CardBlockSize);  // 0~4294967296	 
		
		UART_send_string(USART1," \r\n CardCapacity is ： ");//字符串函数
...
SD_Error SD_Init(void) 
{
  ...
  SD_Error errorstatus = SD_OK;/*重置SD_Error状态*/  
	if(SD_timers==0)
	{
      	SD_DeInit();//SDIO模块端口复位  ---可以不调用
	
				NVIC_Configuration();/* 中断初始化 */
				GPIO_Configuration();/* SDIO 外设底层引脚初始化 */
				SDIO_DeInit(); /*对SDIO的所有寄存器进行复位*/ 
  }

 
	
  errorstatus = SD_PowerON(); /*上电并进行卡识别流程，确认卡的操作电压  */ 
  if (errorstatus != SD_OK)/*如果上电，识别不成功，返回“响应超时”错误 */
  {  
  ...
  if (errorstatus == SD_OK)
  {   
     errorstatus = SD_GetCardInfo(&SDCardInfo); /* 获取SD卡的具体信息  存放在该底层的宏变量--SDCardInfo结构体中*/	
  }
  if (errorstatus == SD_OK)
  {    
...
/* HCLK = 72MHz, SDIOCLK = 72MHz, SDIO_CK = HCLK/(178 + 2) = 400 KHz */
  ...
  SDIO_InitStructure.SDIO_ClockBypass = SDIO_ClockBypass_Disable;/* 不使用bypass模式，直接用HCLK进行分频得到SDIO_CK 超过24M时才使用bypass模式*/	
  SDIO_InitStructure.SDIO_ClockPowerSave = SDIO_ClockPowerSave_Disable;/* 空闲时不关闭时钟电源 */  	
  SDIO_InitStructure.SDIO_BusWide = SDIO_BusWide_1b;/* 初始化的时候暂时先把数据线配置成1根 */	
  SDIO_InitStructure.SDIO_HardwareFlowControl = SDIO_HardwareFlowControl_Disable;/* 使能硬件流控制 */
  SDIO_Init(&SDIO_InitStructure);
  SDIO_SetPowerState(SDIO_PowerState_ON); /* 开启SDIO外设的电源 */  
  SDIO_ClockCmd(ENABLE);  /* 使能 SDIO 时钟 */
...
/********************************************************************************************************/   
  ...
  errorstatus = CmdError();/* 检测是否成功发送cmd0 */
  if (errorstatus != SD_OK)//如果超时会返回	SDIO_CMD0TIMEOUT  反之返回 SD_OK
  {     
    return(errorstatus);
  }
...
/********************************************************************************************************/
  ...
  {
   	// printf( " \r\n SD Card 1.0  \r\n " );	  
    SDIO_CmdInitStructure.SDIO_Argument = 0x00; /* 发命令 CMD55 */
    SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_APP_CMD;
    SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
    SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
    SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
  ...
  SDIO_CmdInitStructure.SDIO_Argument = 0x00;
  SDIO_CmdInitStructure.SDIO_CmdIndex = SD_CMD_APP_CMD;//发送cmd55，用于检测是sd卡还是mmc卡，或是不支持的卡
  SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
  SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
  SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
  SDIO_SendCommand(&SDIO_CmdInitStructure);		
...
SD_Error SD_InitializeCards(void)
{
  ...
  if (SDIO_SECURE_DIGITAL_IO_CARD != CardType)
  {
    ...
    CID_Tab[0] = SDIO_GetResponse(SDIO_RESP1);	/* 将返回的CID信息存储起来 */
...
SD_Error SD_GetCardInfo(SD_CardInfo *cardinfo)
{
  ...
  if ((CardType == SDIO_STD_CAPACITY_SD_CARD_V1_1) || (CardType == SDIO_STD_CAPACITY_SD_CARD_V2_0))
  {
    ...
    cardinfo->CardBlockSize = 512;//默认条件下是 512 Bytes
...
static SD_Error CmdError(void)
{
  ...
  timeout = SDIO_CMD0TIMEOUT;  // SDIO_CMD0TIMEOUT为枚举变量
	
  while ((timeout > 0) && (SDIO_GetFlagStatus(SDIO_FLAG_CMDSENT) == RESET))	/*检查命令是否已发送*/
  {
    timeout--;
  }
  if (timeout == 0)
